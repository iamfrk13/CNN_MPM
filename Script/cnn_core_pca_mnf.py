...
# Paths 
train_dir = r"E:/Sentinel_Dataset/cnn/model_datasets/core_pca_mnf_dataset/train"
val_dir   = r"E:/Sentinel_Dataset/cnn/model_datasets/core_pca_mnf_dataset/validation"
test_dir  = r"E:/Sentinel_Dataset/cnn/model_datasets/core_pca_mnf_dataset/test"
...
# Input layer accepting images of shape 64x64 with 13 channels
inputs = keras.Input(shape=(64, 64, 13))
...
callbacks = [
    keras.callbacks.ModelCheckpoint(filepath="convnet_core_pca_mnf_scratch.keras", save_best_only=True, monitor="val_loss")
]
...
test_model = keras.models.load_model("convnet_core_pca_mnf_scratch.keras")
...
# Save & name the best model as "convnet_core.keras" automatically using a callback
callbacks = [
    keras.callbacks.ModelCheckpoint(filepath="convnet_core_pca_mnf_aug.keras", save_best_only=True, monitor="val_loss")
]
...
# Load the best saved model and evaluate on test dataset
test_model = keras.models.load_model("convnet_core_pca_mnf_aug.keras")
test_loss, test_acc = test_model.evaluate(test_dataset)
print(f"Test accuracy: {test_acc:.3f}")
...
x = layers.Conv2D(10, (1, 1), padding="same", activation="linear")(x)  # map 13→10 bands
...
callbacks = [
    keras.callbacks.ModelCheckpoint(filepath="convnet_core_pca_mnf_finetuned_1.keras", save_best_only=True, monitor="val_loss")
]
...
# Load the best saved model and evaluate on test dataset
test_model = keras.models.load_model("convnet_core_pca_mnf_finetuned_1.keras")
test_loss, test_acc = test_model.evaluate(test_dataset)
print(f"Test accuracy: {test_acc:.3f}")
...
model = keras.models.load_model("convnet_core_pca_mnf_finetuned_1.keras")
...
fine_tune_callbacks = [
    keras.callbacks.ModelCheckpoint(
        filepath="convnet_core_pca_mnf_finetuned_2.keras",
        save_best_only=True,
        monitor="val_loss")
]
...
# Load the best saved model and evaluate on test dataset
test_model = keras.models.load_model("convnet_core_pca_mnf_finetuned_2.keras")
test_loss, test_acc = test_model.evaluate(test_dataset)
print(f"Test accuracy: {test_acc:.3f}")
